...
Configuration via environment variables:
    SE_MCP_DIAGRAM_RENDERER: "mermaid_ink" (default), or "none" to skip rendering
"""
...
def get_renderer_mode() -> str:
    ...
    return os.getenv("SE_MCP_DIAGRAM_RENDERER", "mermaid_ink").strip().lower()
...
def render_mermaid_to_file(
    ...
    *,
    ...
    if get_renderer_mode() == "none":
        logger.debug("Diagram rendering disabled (SE_MCP_DIAGRAM_RENDERER=none)")
        return False
    ...
    try:
        request = Request(url, headers={"User-Agent": "se-lifecycle-mcp/1.0"})
