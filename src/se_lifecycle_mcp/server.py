"""FastMCP server for the SE Lifecycle MCP."""
...
mcp = FastMCP(
    "se-lifecycle-mcp",
    instructions=(
        "SE Lifecycle MCP turns a one-line product idea into a disciplined "
        "software engineering build flow. Use start_product_build first, then "
        "advance through communication, requirements, modeling, design, planning, "
        "construction, testing, and deployment. Do not start implementation until "
        "requirements, modeling, design, and planning gates are ready. Generate "
        "Mermaid diagrams when the user permits diagrams."
    ),
)
...
try:
    ...
    sys.stderr.write(f"SE Lifecycle MCP provider warning: {exc}\n")
