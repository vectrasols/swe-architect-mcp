"""LLM provider factory for the SE Lifecycle MCP."""
...
_PROVIDER_REGISTRY: dict[str, tuple[str, str]] = {
    "anthropic": ("se_lifecycle_mcp.llm.anthropic", "AnthropicProvider"),
    "openai": ("se_lifecycle_mcp.llm.openai", "OpenAIProvider"),
    "google": ("se_lifecycle_mcp.llm.google", "GoogleProvider"),
}
...
def create_provider(
    ...
    if not name:
        raise ValueError(
            ...
            "  pip install se-lifecycle-mcp[anthropic]  # or [openai] or [google] or [all]"
        )
    ...
    try:
        ...
        raise ImportError(
            ...
            f"Install with: pip install se-lifecycle-mcp[{name}]"
        ) from e
    ...
    sys.stderr.write(
        f"SE Lifecycle MCP using {provider.provider_name} "
